...
class Game:
    ...
    def setup_room_one(self):
        ...
        for item in walls:
            wall = Wall(item[0], item[1], item[2], item[3], self.color.blue)
            wall_list.add(wall)
    ...
    def start_game(self):
        ...
        pl = len(ghost_one_directions) - 1
        bl = len(ghost_two_directions) - 1
        il = len(ghost_three_directions) - 1
        cl = len(ghost_four_directions) - 1
        ...
        # default locations for Pacman and ghost
        w = 303 - 16  # Width
        p_h = 7 * 60 + 19
        m_h = 4 * 60 + 19
        b_h = 3 * 60 + 19
        ...
        while not done:
            ...
            returned = ghost_one.ghost_change_speed(ghost_one_directions, False, p_turn, p_steps, pl)
            p_turn = returned[0]
            p_steps = returned[1]
            ghost_one.ghost_change_speed(ghost_one_directions, False, p_turn, p_steps, pl)
            ghost_one.update(wall_list, False)

            returned = ghost_two.ghost_change_speed(ghost_two_directions, False, b_turn, b_steps, bl)
            b_turn = returned[0]
            b_steps = returned[1]
            ghost_two.ghost_change_speed(ghost_two_directions, False, b_turn, b_steps, bl)
            ghost_two.update(wall_list, False)

            returned = ghost_three.ghost_change_speed(ghost_three_directions, False, i_turn, i_steps, il)
            i_turn = returned[0]
            i_steps = returned[1]
            ghost_three.ghost_change_speed(ghost_three_directions, False, i_turn, i_steps, il)
            ghost_three.update(wall_list, False)

            returned = ghost_four.ghost_change_speed(ghost_four_directions, 'ghost_four', c_turn, c_steps, cl)
            c_turn = returned[0]
            c_steps = returned[1]
            ghost_four.ghost_change_speed(ghost_four_directions, 'ghost_four', c_turn, c_steps, cl)
            ...
            text = self.font.render('Score: ' + str(score) + '/' + str(bll), True, self.color.red)
    ...
    def do_next(self, message, left, all_sprites_list, block_list, ghost_list, pacman_collide, wall_list, gate):
        ...
        while True:
            ...
            w_surface.set_alpha(10)  # alpha level
            w_surface.fill((128, 128, 128))  # this fills the entire surface
            self.screen.blit(w_surface, (100, 200))  # (0,0) are the top-left coordinates

            # Won or lost
            text1 = self.font.render(message, True, self.color.white)
            self.screen.blit(text1, [left, 233])

            text2 = self.font.render('To play again, press ENTER.', True, self.color.white)
            self.screen.blit(text2, [135, 303])
            text3 = self.font.render('To quit, press ESCAPE.', True, self.color.white)
